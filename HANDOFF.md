...
- 주사 목록의 장 표시 옆에는 차트에서 가장 최근에 입력된 체중을 kg으로 표시한다. 체중 행과
  환자 정보의 체중 필드를 차례로 확인하며 값이 없으면 `- kg`로 표시한다.
...
- 주사 목록은 날짜별로 마지막 확인 기준을 브라우저 `localStorage`에 저장한다. 다시 불러왔을 때
  ...
  차트의 `SKIPPED`/`CANCELLED` 칸은 `[시간] 취소`로 표시하고, 약물의 마지막 시간이 취소라면
  같은 줄에 `마지막 시간 취소`를 붙인다. 미연장 차트 예측에는 취소 칸을 복사하지 않는다.
