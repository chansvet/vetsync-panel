...
- 조건부 처치는 맨 아래 따로 모은다.
- 정규 회진 시간은 17시, 21시, 1시, 9시다. 그 외 시간은 밑줄을 쳐서 눈에 띄게 한다.
  화면에서는 `<u>`, 복사한 글에서는 앞뒤를 `_` 로 감싼다.
- 투여 경로가 SC 또는 IM으로 명시된 주사는 처치 한 줄 전체를 굵게+밑줄로 강조한다.
  화면에서는 `<strong>`과 밑줄, 복사한 글에서는 앞뒤를 `**__`, `__**`로 감싼다.
...
0시부터 15시까지 오더를 그대로 다음날로 복사해서 보여준다. 그 환자는 이름 옆에
`(미연장)` 을 붙이고, 오늘 시간과 예상 시간을 한 줄에 이어서 표시한다.
