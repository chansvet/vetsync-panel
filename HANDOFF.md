...
없는 처치가 목록에 나온다. `PLANNED`, `COMPLETED`, `IN_PROGRESS`, `SKIPPED` 만 세야 한다.
...
- BID의 정규 시간은 21시·9시, TID는 17시·1시·9시다. 각 빈도의 정규 시간이 아닌 시간은
  ...
  화면에서는 `<u>`, 복사한 글에서는 앞뒤를 `_` 로 감싼다.
- 투여 경로가 SC 또는 IM으로 명시된 주사는 `SC` 또는 `IM` 글자에만 밑줄을 친다.
...
- 주사 탭의 `감시 켜기`를 누르면 3분마다 캐시를 무시하고 최신 차트를 조회한다. 변경 건이 새로
  발견되면 환자정보 없이 변경 건수만 브라우저 알림으로 띄우고 화면 목록도 갱신한다. 같은 상태로는
  반복 알림하지 않는다. 감시 설정은 `vetsync-injection-monitor-v1`에 저장되지만 패널을 닫으면
  타이머는 멈추며, 다음에 패널을 열 때 설정에 따라 다시 시작한다.
  ...
  for (const d of ['2026-09-08', '2026-09-09', '2026-09-10']) {   // 보고 싶은 날짜로 바꾼다
    ...
    details.forEach((x) =>
      (x.sections || []).filter((s) => s.section === 'TREATMENT')
        .flatMap((s) => s.rows || [])
        .forEach((r) => {
          if ((r.cells || []).some((c) => LIVE.includes(c.status))) names.add(r.displayName.trim());
        })
    );
  }
