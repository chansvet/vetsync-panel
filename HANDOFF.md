...
- 오후 6시 전에는 당일 오전 9시 차트를, 오후 6시 이후에는 다음 날 오전 9시 차트를 기준으로
  목록을 만든다. 오후 6시 기준 입원 중인데 다음 날 활성 차트가 없으면 실제 채혈 목록과 분리한
  `연장되지 않음` 구역에만 표시한다.
...
- 주사 목록은 환자 이름 뒤 괄호 안에 체중, 환자번호, 품종 순으로 표시한다.
  오늘 차트의 체중 행과 환자 정보 필드를 먼저 확인하고, 오늘 값이 없을 때만 어제 차트의 가장 최근 값을 쓴다. 어제 보다 오래된 차트는 조회하지 않는다.
  값이 없으면 `- kg`로 표시한다. 어제 기준 체중을 저장한 후 오늘 새 체중이 입력되어 달라지면 환자 이름 줄에 이전 체중→현재 체중으로 표시하고 업데이트로 집계한다. 예: `뽀삐 (5.2 kg · #257008 · 푸들)`.
...
- 자동 감시는 없다. 주사 탭을 열거나 `새로 확인`을 누를 때 최신 차트를 조회한다.
  ...
  `새로 확인` 버튼 옆에는 변경 유무와 무관하게 확인 시각을 항상 표시한다. 첫 확인은
  `현재 확인 HH:MM`, 이후 조회는 `이전 확인 HH:MM → 현재 확인 HH:MM` 형식이다.
